...
def test_get_comment_body_no_user_id():
    ...
    response = get_comment_body("github", SIGN_URL, [], [(GITHUB_FAKE_SHA, None), (GITHUB_FAKE_SHA_2, None)])
    expected = (
        f"<ul><li>"
        + FAILED
        + "The commit ("
...
def test_get_comment_body_cla_fail_no_user_id_and_user_id():
    ...
    """
    # case with missing list with authors
    author_name = "wanyaland"
    response = get_comment_body("github", SIGN_URL, [], [(GITHUB_FAKE_SHA, author_name), (GITHUB_FAKE_SHA_2, None)])
    expected = (
        f"<ul><li>"
        + "["
        + FAILED
